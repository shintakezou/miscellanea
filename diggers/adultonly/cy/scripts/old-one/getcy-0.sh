#!/bin/bash
#IFS=" "
# 20070522: stripped oll clue to the hot site: just the idea is important...
#           but you searcher can trace all back just looking around.
baseaddr="http://www.PUT HERE THE ADDRESS"
...
# a generic function that stores cooking and reuse them (for curl sake)
function zurl {
    if [ -e $fcookie ]; then
...
function browse {
  ...
  for findice in `cat $1 |getaddrs |egrep "[0-9]*\.html?$"`
  do
...
do
  ...
  for sindice in `cat $indici |getaddrs |egrep "p[a-z]*/00[0-9]\.html?$" |sort |uniq`
  do
